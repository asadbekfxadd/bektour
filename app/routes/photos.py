"""
BekTour — Premium SVG City Art + Real Photo Support
Each city gets a unique, detailed architectural illustration as fallback.
If real photos exist in app/static/img/destinations/<city>/, those are
served instead — drop files in and they appear automatically, no code change needed.
"""
import os
import random
...
def _real_photos_for(city_key):
    """Return list of real photo filenames available for a city, sorted naturally.
    Looks in app/static/img/destinations/<city_key>/
    Empty list if folder doesn't exist or has no images yet."""
    try:
        folder = os.path.join(current_app.static_folder, 'img', 'destinations', city_key)
...
def city_photo(city):
    ...
    variant = int(request.args.get('v', 0))
    city_key = city.lower().replace('-valley', '').replace('-', '').replace(' ', '').strip()

    # ── Try real uploaded photo first ──
    real_files = _real_photos_for(city_key)
    if real_files:
        idx = variant % len(real_files)
        filename = real_files[idx]
        folder = os.path.join('img', 'destinations', city_key)
        return send_from_directory(
            os.path.join(current_app.static_folder, folder),
            filename,
            max_age=604800
        )

    # ── Fallback: generated SVG art ──
