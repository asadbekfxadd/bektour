...
def media_upload():
    agency = get_agency_or_redirect()
    if not agency: return redirect(url_for('b2b.register'))
    # Note: actual file upload requires storage config; this records metadata for demo/dev
    filename = request.form.get('filename', 'file')
    file_type = request.form.get('file_type', 'image')
    folder = request.form.get('folder', 'General')
    asset = MediaAsset(agency_id=agency.id, filename=filename,
                        file_url='/photo/samarkand?w=400&h=300',
                        file_type=file_type, folder=folder, file_size_kb=120)
